...
class AddBackgroundNoise(BaseWaveformTransform):
    ...
    * https://github.com/microsoft/DNS-Challenge/
    ...
    @staticmethod
    def _load_sound(file_path, sample_rate, mono=True, offset=0, duration=None):
    ...
    def randomize_parameters(self, samples: NDArray[np.float32], sample_rate: int):
        ...
        if self.parameters["should_apply"]:
            ...
            info = soundfile.info(self.parameters["noise_file_path"])
            min_noise_offset = 0
            max_noise_offset = max(0, info.duration - num_samples / sample_rate)
            self.parameters["noise_start_index"] = random.uniform(
    ...
    def apply(
        ...
        noise_sound, _ = self._load_sound(
            self.parameters["noise_file_path"], sample_rate, offset=self.parameters["noise_start_index"], duration= len(samples) / sample_rate
        )
